// Compile contracts/AgentJob.sol (solc 0.8.26, optimizer 200) and deploy it to Arc
// testnet. No constructor args. Writes oracle/job-deployment.json. Mirrors deploy-validator.mjs.
...
async function withRetry(label, fn, tries = 8) {
  ...
  for (let i = 0; i < tries; i++) {
    try {
      ...
      console.log(`  (${label}) retry ${i + 1}/${tries}…`);
    }
  }
  throw lastErr;
}
...
const SOL = new URL("../contracts/AgentJob.sol", import.meta.url).pathname;
const OUT_DIR = new URL("../oracle/", import.meta.url).pathname;
const DEPLOYMENT = OUT_DIR + "job-deployment.json";
...
const input = {
  language: "Solidity",
  sources: { "AgentJob.sol": { content: source } },
  settings: { optimizer: { enabled: true, runs: 200 }, outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const out = JSON.parse(solc.compile(JSON.stringify(input)));
const fatal = (out.errors || []).filter((e) => e.severity === "error");
if (fatal.length) { console.error("Solc errors:\n" + fatal.map((e) => e.formattedMessage).join("\n")); process.exit(1); }
const c = out.contracts["AgentJob.sol"]["AgentJob"];
console.log("Compiled AgentJob (solc " + solc.version() + ").");
...
console.log("Deployer:", wallet.address, "| balance:", bal.toString());
if (bal === 0n) { console.error("Balance 0 — fund at https://faucet.circle.com (Arc Testnet)."); process.exit(1); }
...
console.log("\nAgentJob deployed at:", addr);
console.log("   " + EXPLORER + "/address/" + addr);

if (!existsSync(OUT_DIR)) mkdirSync(OUT_DIR, { recursive: true });
writeFileSync(
  DEPLOYMENT,
  JSON.stringify({ address: addr, deployer: wallet.address, chainId: Number(ARC_CHAINID), deployTx: deployTx.hash, deployedAt: new Date().toISOString() }, null, 2) + "\n",
);
console.log("\nWrote oracle/job-deployment.json");
