...
//
//   node script/live-loop-mainnet.mjs [wallet]      (Arc mainnet, chainId 5042)
//
// One key plays client, provider and validator here so the demo needs a single funded wallet; in
// production those are three parties. Amounts are dust USDC (see VALUES). Receipts go to
// deployments/arc-mainnet/live-loop-<timestamp>.json.
...
import { withRetry, routeHash, SETTLEMENT_ABI } from "../lib/arc.mjs";

// Arc mainnet: USDC is the native gas token, so bond / escrow / payment values below are USDC.
const NETWORK = { name: "Arc mainnet", chainId: 5042n };
const NETWORK_NAME = "arc-mainnet";
...
const addr = (name) => {
  const p = new URL(`../deployments/arc-mainnet/${name}.json`, import.meta.url).pathname;
  if (!existsSync(p)) throw new Error(`missing ${p}; run script/deploy-mainnet.mjs first`);
  return JSON.parse(readFileSync(p, "utf8")).address;
};

const e = env();
const provider = new JsonRpcProvider(e.ARC_MAINNET_RPC || "https://rpc.mainnet.arc.io", undefined, { staticNetwork: Network.from(NETWORK.chainId) });
...
const providerWallet = new Wallet(keccak256(toUtf8Bytes(`${e.PRIVATE_KEY}:predge-loop-provider`)), provider);
...
console.log(`operator ${wallet.address} | balance ${formatEther(await provider.getBalance(wallet.address))} USDC\n`);
...
const out = new URL(`../deployments/arc-mainnet/live-loop-${Date.now()}.json`, import.meta.url).pathname;
...
console.log(`\nreceipts -> ${out}`);
console.log(`provider (separate key) ${providerWallet.address}`);
